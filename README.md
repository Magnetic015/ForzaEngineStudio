...
- **AI pre‑processing (optional)** — edit the source image with a third‑party image model **before** rendering, through an OpenAI‑compatible gateway. Set the gateway base URL via the `FES_AI_BASE_URL` environment variable (e.g. `https://your-gateway.example/v1`) before launching the app — no endpoint is hardcoded. The composer is Semi's `AIChatInput`; click the model trigger to set your **API key** and pick a model:
...
To produce a distributable Windows installer (MSI / NSIS), run from the repo root:

```powershell
.\build-release.ps1 -Version 0.2.0
```

This freezes the Python engine with PyInstaller, bundles it as a Tauri resource, builds the front‑end and Rust shell, and emits installers under `src-tauri\target\release\bundle\`. The full procedure, versioning, sidecar‑packaging design and troubleshooting are in **[RELEASE.md](RELEASE.md)**.
...
- **Sidecar path** — in dev the sidecar runs from `python/.venv` (resolved via `CARGO_MANIFEST_DIR`); a packaged install instead runs the PyInstaller‑frozen `fes-engine` shipped as a Tauri resource. See [RELEASE.md](RELEASE.md).
...
- **AI 预处理（可选）**——渲染**前**用第三方图像模型编辑源图，经 OpenAI 兼容网关。网关地址通过环境变量 `FES_AI_BASE_URL` 配置（例如 `https://your-gateway.example/v1`），启动应用前设置即可——代码中不再硬编码任何端点。输入框是 Semi 的 `AIChatInput`；点模型触发器设置 **API Key** 并选模型：
  - `gemini-3.1-flash-image`（`/chat/completions`）
  - `plus/gpt-image-2`（`/images/edits`）
  - `grok-imagine-image-lite`（`/chat/completions`）
...
产出可分发的 Windows 安装包（MSI / NSIS），在项目根目录运行：

```powershell
.\build-release.ps1 -Version 0.2.0
```

该脚本用 PyInstaller 冻结 Python 引擎、以 Tauri 资源打包，再构建前端与 Rust 壳，安装包产物落在 `src-tauri\target\release\bundle\`。完整流程、版本管理、sidecar 打包设计与排错见 **[RELEASE.md](RELEASE.md)**。
...
- **sidecar 路径**——开发态从 `python/.venv` 跑 sidecar（靠 `CARGO_MANIFEST_DIR` 定位）；安装包则改跑 PyInstaller 冻结、以 Tauri 资源分发的 `fes-engine`。见 [RELEASE.md](RELEASE.md)。

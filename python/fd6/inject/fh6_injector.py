    ...
    def attach(self) -> None:
        if self.pid is None:
            ...
            if self.pid is None:
                ...
                raise RuntimeError(
                    f"{self.profile.label} is not running, OR this app is running with lower "
                    f"privileges than the game. If the game IS open, close this app and "
                    f"re-launch it as Administrator. The game's process memory is inaccessible "
                    f"from a non-elevated app even when both processes are running. "
                    f"(Looked for: {names}.)"
                )
